...
class ProductManager {
  ...
  cargarProductos () {
    try {
      ...
      console.log('No se encontro archivo de productos, se procederá a generar uno cuando se agreguen productos')
      return []
    }
  }
  ...
  getProductById (idBuscado) {
    const busqueda = this.products.find((objeto) => objeto.id === idBuscado)
  ...
  deleteProduct (idBuscado) {
    ...
    if (busqueda) {
      const objetoEliminado = this.products.filter((objeto) => objeto.id !== idBuscado)
